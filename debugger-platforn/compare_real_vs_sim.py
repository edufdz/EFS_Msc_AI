#!/usr/bin/env python3
"""
Real-vs-Simulated failure comparison.
...
# Fixed up front, before any comparison was run, so the reachability verdicts
# below cannot be tuned to the results.
# ----------------------------------------------------------------------
...
def write_report(results: Dict[str, Any], out_dir: Path) -> None:
    ...
    lines += [
        "",
        "_Generated by compare_real_vs_sim.py. The pre-registered design and the "
        "validity threats are documented in the script's module docstring and in "
        "its REACHABILITY table._",
    ]
