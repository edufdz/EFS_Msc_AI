...
- Both corpora are scored by the same **rule-based** scorer (scorer parity by construction, no LLM in the loop), so all category counts inherit the scorer's precision.
